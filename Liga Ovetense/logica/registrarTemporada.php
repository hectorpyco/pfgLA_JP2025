<?php
    ...
    $sql_verificar = "SELECT id FROM temporadas WHERE UPPER(descripcion) = UPPER('$desc')";
    $resultado = mysqli_query($conexion, $sql_verificar);

    if (mysqli_num_rows($resultado) > 0) {
        $response["exito"] = false;
        $response["mensaje"] = "Ya existe una temporada con esa descripción.";
    } else {
        // Si no existe, se inserta el nuevo registro
        $sql = "INSERT INTO temporadas (id, descripcion, estado) VALUES (null, '$desc', 'ACTIVO')";
