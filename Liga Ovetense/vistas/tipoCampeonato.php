...
<html lang="en">
  ...
  <body>
                  ...
                  <div class='layout-page'>
                    ...
                    if ($r==1) {
                      ...
                      echo "<script>
                        swal.fire({
                          ...
                          text: 'El Tipo de Campeonato ya se encuentra registrado',  
                        })
                        </script>";        
                    }else if ($r==3) {
                      echo "<script>
                    ...
                    <div class='content-wrapper'>
                      <div class='container-xxl flex-grow-1 container-p-y'>
                        <h4 class='fw-bold py-3 mb-4'><span class='text-muted fw-light'>Campeonatos /</span> Tipo de Campeonato</h4>
                        <div class='row'>
                          <div class='col-md-12'>
                            <ul class='nav nav-pills flex-column flex-md-row mb-3'>
                              <li class='nav-item'>
                                <a class='nav-link active' href='javascript:void(0);'>Listado de Tipo de Campeonato</a>
                              </li>
                              <li class='nav-item'>
                                <button type='button' class='nav-link btn btn-link' disabled data-bs-toggle='modal' data-bs-target='#modalTipoCampeonato'>
                                  Registrar Nuevo Tipo de Campeonato
                                </button>
                              </li>
                            </ul>
                            <div class='card mb-4'>
                              <h5 class='card-header'>Listado de Tipo de Campeonato</h5>
                              <div class='card-body'>
                                ...
                                $sql1 = "SELECT * from tipo_campeonato
                                         WHERE estado = 'ACTIVO'
                                         ORDER BY id DESC";
                                ...
                                if (mysqli_num_rows($res1) > 0) {
                                  echo "<div class='table-responsive text-nowrap' style='overflow-x: auto; max-height: 800px;'>
                                    <table class='table table-hover'>
                                      ...
                                      <tbody  id='tablaTipoCampeonato' class='table-border-bottom-0 buscar'>";
                                        while ($reg1 = mysqli_fetch_array($res1)) {
                                          echo "<tr class='fila-editar' data-id='{$reg1['id']}' data-descripcion='{$reg1['descripcion']}' style='cursor: pointer; text-align: center;'>
                                            <td>{$reg1['id']}</td>
                                            <td>{$reg1['descripcion']}</td>
                                            <td>
                                              <a class='btn btn-sm btn-danger' href='../logica/eliminarTipoCampeonato.php?usuario={$uActual}&id={$reg1['id']}'>
                      ...
                      <div class='modal fade' id='modalTipoCampeonato' tabindex='-1' aria-labelledby='modalTipoCampeonatoLabel' aria-hidden='true'>
                        <div class='modal-dialog'>
                          <form id='formTipoCampeonato' action='../logica/registrarTipoCampeonato.php' method='POST'>
                            <input type='hidden' name='usuario' value='$uActual'>
                            <div class='modal-content'>
                              <div class='modal-header'>
                                <h5 class='modal-title' id='modalTipoCampeonatoLabel'>Registrar Nuevo Tipo de Campeonato</h5>
                      ...
                      <div class='modal fade' id='modalEditar' tabindex='-1' aria-labelledby='modalEditarLabel' aria-hidden='true'>
                        <div class='modal-dialog'>
                          <form action='../logica/editarTipoCampeonato.php' method='POST'>
                            <div class='modal-content'>
                              <div class='modal-header'>
                                <h5 class='modal-title' id='modalEditarLabel'>Editar Tipo de Campeonato</h5>
                                <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Cerrar'></button>
                              </div>
                              <div class='modal-body'>
                        ...
                        <div class='row'>
                          <div class='col-md-12'>
                            <ul class='nav nav-pills flex-column flex-md-row mb-3'>
                              ...
                              <li class='nav-item'>
                                <button type='button' class='nav-link btn btn-link' data-bs-toggle='modal' data-bs-target='#modalTipoCampeonato'>
                                  Registrar Nuevo Tipo de Campeonato
                                </button>
                              </li>
                            </ul>
    ...
    <script>
      document.getElementById("formTipoCampeonato").addEventListener("submit", function(e) {
        e.preventDefault(); // Evita el envío normal del formulario

        const form = e.target;
        const datos = new FormData(form);

        fetch("../logica/registrarTipoCampeonato.php", {
          method: "POST",
          body: datos
        })
        .then(res => res.json())
        .then(data => {
          if (data.exito) {
            // Cerrar el modal
            const modal = bootstrap.Modal.getInstance(document.getElementById('modalTipoCampeonato'));
            modal.hide();
            ...
            const nuevaFila = `
              <tr class="fila-editar" data-id="${data.id}" data-descripcion="${data.descripcion}" style="cursor: pointer; text-align: center;">
                <td>${data.id}</td>
                <td>${data.descripcion}</td>
                <td>
                  <a class='btn btn-sm btn-danger' href='../logica/eliminarTipoCampeonato.php?usuario=${data.usuario}&id=${data.id}'>
                    <i class='bx bx-trash'></i>
                  </a>
                </td>
              </tr>
            ...
            document.getElementById("tablaTipoCampeonato").insertAdjacentHTML("afterbegin", nuevaFila);
        ...
        .catch(err => {
          alert("Ocurrió un error al registrar el tipo de campeonato.");
          console.error(err);
        });
      });
    </script>
