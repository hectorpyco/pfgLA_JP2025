                  ...
                  <div class='layout-page'>
                    ...
                    <div class='content-wrapper'>
                      <div class='container-xxl flex-grow-1 container-p-y'>
                        <h4 class='fw-bold py-3 mb-4'><span class='text-muted fw-light'>Auditoría /</span> Historial Futbolístico</h4>
                        <div class='row'>
                          <div class='col-md-12'>
                            <ul class='nav nav-pills flex-column flex-md-row mb-3'>
                              <li class='nav-item'>
                                <a class='nav-link active' href='javascript:void(0);'>Acciones sobre los Historiales Futbolísticos</a>
                              </li>
                            </ul>
                            <div class='card mb-4'>
                                ...
                                <div class='table-responsive' style='overflow-x: auto; max-height: 600px;'>
                                  <table class='table table-bordered table-striped align-middle' id='tablaAuditoria'>
                                    ...
                                    <tbody>";
                                        ...
                                        FROM auditoria_historiales a
                                        LEFT JOIN historiales_futbolistico h ON a.idhistorial = h.id
                                        LEFT JOIN jugadores j ON h.idjugador = j.id
                                        ORDER BY a.fecha_hora DESC";
                                        $res = mysqli_query($conexion, $sql);
                                        if (mysqli_num_rows($res) > 0) {
                                          while ($row = mysqli_fetch_assoc($res)) {
                                            $jugador = isset($row['nombre_jugador']) ? $row['nombre_jugador'] . ' ' . $row['apellido_jugador'] : 'Desconocido';
                                            echo "
                                              <tr>
                                                <td>" . ($row['id'] ?? '-') . "</td>
                                          ...
                                          echo "
                                          <tr>
                                            <td colspan='7' class='text-center'>No hay registros en la auditoría.</td>
                                          </tr>";
                                        }
                                      echo"
                                    </tbody>
    ...
    <script>
      document.addEventListener("DOMContentLoaded", function () {
        ...
        function filtrarTabla() {
          ...
          filas.forEach(fila => {
            const fecha = fila.children[4].textContent.trim().substring(0, 10); // Fecha y Hora
            const accionFila = fila.children[2].textContent.trim().toUpperCase(); // Acción
